...
namespace LinqCaseStudy
{
    internal class Query9
    {
        static void Main()
        {
            ...
            var ans9 = productList.Where(p => p.ProCategory == "FMCG").OrderByDescending(p => p.ProMrp).FirstOrDefault();
            Console.WriteLine($"\t{ans9.ProCode}\t{ans9.ProName}\t{ans9.ProCategory}\t{ans9.ProMrp}");
        }
    }
}
